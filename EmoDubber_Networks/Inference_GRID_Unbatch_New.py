...
def cli():
    ...
    vocoder = load_vocoder(args.vocoder, paths["EmoDubber_16Khz_Vocoder"], device)
    ...
    with open(
        ...
        for line in tqdm(f.readlines()):
            ...
            base_name_path, args.text, phoneme = line.strip("\n").split("|")

            base_name = base_name_path.split('/')[-1].split('.wav')[0]
            
            base_n = base_name_path.split('/')[-2]
            
            texts = get_texts(args)
            
            # args.Refence_audio,
            Speaker_GE2E_ID_path = os.path.join(
                            args.Refence_audio, base_n, base_name+'.npy'
                        ) 
            
            Speaker_GE2E_ID = torch.from_numpy(np.load(Speaker_GE2E_ID_path)).unsqueeze(0).to(device)

            lip_embedding_path = os.path.join(
                            args.Silent_Lip,
                            "{}-face-{}.npy".format(base_n, base_name),
                        )
            ...
            VA_path = os.path.join(
                            args.Silent_Face,
                            "{}-feature-{}.npy".format(base_n, base_name),
                        ) 
            feature_256 = np.load(VA_path)
            feature_256 = torch.from_numpy(feature_256[:lip_embedding.shape[1], :]).unsqueeze(0).float().to(device)
            
            unbatched_synthesis(args, device, model, vocoder, texts, Speaker_GE2E_ID, lip_embedding, lip_embedding_length, base_name_path, "{}-{}".format(base_n, base_name), phoneme,feature_256)
...
def unbatched_synthesis(args, device, model, vocoder, texts, Speaker_GE2E_ID, lip_embedding, lip_embedding_length, base_name_path, base_name, phoneme, feature_256):
    total_rtf = []
    total_rtf_w = []
    for i, text in enumerate(texts):
        ...
        location = save_to_folder(base_name, output, os.path.join(args.output_folder, "Setting1_GRID_outputWav_Step_{}".format(args.checkpoint_path.split('/')[-1].split('=')[-1].split('.ckpt')[0])))
