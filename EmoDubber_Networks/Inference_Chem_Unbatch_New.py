    ...
    with open(
        ...
        for line in tqdm(f.readlines()):
            ...
            spk_path = os.path.join(
                        args.Refence_audio,
                        "SPK-{}.npy".format(base_name_path.split('/')[-1].split(".wav")[0]),
                    )
            ...
            unbatched_synthesis(args, device, model, vocoder, texts, Speaker_GE2E_ID, lip_embedding, lip_embedding_length, base_name_path, base_name, phoneme,feature_256)
...
def unbatched_synthesis(args, device, model, vocoder, texts, Speaker_GE2E_ID, lip_embedding, lip_embedding_length, base_name_path, base_name, phoneme, feature_256):
    ...
    for i, text in enumerate(texts):
        ...
        location = save_to_folder(base_name, output, os.path.join(args.output_folder, "Setting1_Chem_outputWav_Step_{}".format(args.checkpoint_path.split('/')[-1].split('=')[-1].split('.ckpt')[0])))
