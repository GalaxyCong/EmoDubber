...
def cli():
    ...
    parser.add_argument(
        "--Set2_list",
        type=str,
        default = 'Refrence_list_self.txt',
    ...
    with open(
        ...
        for line in tqdm(f.readlines()):
            ...
            reference_audio = data_dict[base_n+'-'+base_name]

            Speaker_GE2E_ID_path = os.path.join(
                            args.Refence_audio, base_n, reference_audio.split('-')[-1]+'.npy'
                        ) 
            ...
            unbatched_synthesis(args, device, model, vocoder, denoiser, texts, Speaker_GE2E_ID, lip_embedding, lip_embedding_length, base_name_path, "{}-{}".format(base_n, base_name), phoneme,feature_256)
...
def unbatched_synthesis(args, device, model, vocoder, denoiser, texts, Speaker_GE2E_ID, lip_embedding, lip_embedding_length, base_name_path, base_name, phoneme, feature_256):
    ...
    for i, text in enumerate(texts):
        ...
        location = save_to_folder(base_name, output, os.path.join(args.output_folder, "Setting2_GRID_outputWav_Step_{}".format(args.checkpoint_path.split('/')[-1].split('=')[-1].split('.ckpt')[0])))
