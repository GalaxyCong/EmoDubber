...
class LRS(Dataset):
    ...
    def __init__(self, batch_size, collate_fn, version="LRS2", img_mean=(0.5,), img_std=(0.5,), crop_mouth=True, root="datasets", shuffle=True, ascending=False, mode="pretrain+train+val", load_audio=True, load_video=True, video_transform=None, audio_transform=None, download=False, prepare=False, workers_prepare=-1, video_max_length=None, audio_max_length=None, label_max_length=None, tokenizer_path="datasets/LRS3/tokenizerbpe256.model", mean_face_path="media/20words_mean_face.npy", align=False):
        ...
        if version == "LRS2":
            ...
            self.paths = []
            if "pretrain" in mode:
                with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", "pretrain.txt")) as f:
                    for line in f.readlines():
                        self.paths.append(os.path.join(root, "LRS2", "mvlrs_v1", "pretrain", line.replace("\n", "")))
            if "train" in mode:
                with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", "train.txt")) as f:
                    for line in f.readlines():
                        self.paths.append(os.path.join(root, "LRS2", "mvlrs_v1", "main", line.replace("\n", "")))
            if "val" in mode:
                with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", "val.txt")) as f:
                    for line in f.readlines():
                        self.paths.append(os.path.join(root, "LRS2", "mvlrs_v1", "main", line.replace("\n", "")))
            if "test" in mode:
                with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", "test.txt")) as f:
                    for line in f.readlines():
    ...
    def create_corpus(self, mode):
        ...
        if self.version == "LRS2":
            ...
            if "pretrain" == mode:
                with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", "pretrain.txt")) as f:
                    for line in tqdm(f.readlines()):
                        ...
                        with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos/pretrain", line.replace("\n", "") + ".txt"), "r") as f:
            ...
            if "train" == mode:
                with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", "train.txt")) as f:
                    for line in tqdm(f.readlines()):
                        with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos/main", line.replace("\n", "") + ".txt"), "r") as f:
                            line = f.readline()[7:].replace("{NS}", "").replace("{LG}", "").lower()
                            corpus_file.write(line)
            ...
            if "test" == mode:
                with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", "test.txt")) as f:
                    for line in tqdm(f.readlines()):
                        with open(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos/main", line.split()[0] + ".txt"), "r") as f:
                            line = f.readline()[7:].replace("{NS}", "").replace("{LG}", "").lower()
                            corpus_file.write(line)
    ...
    class FilterDataset:
        ...
        def __getitem__(self, idx):
            ...
            file_path = os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", file_path.split("/")[-3], file_path.split("/")[-2], file_path.split("/")[-1])
        ...
        if self.video_max_length==None and self.audio_max_length==None and self.label_max_length==None:
            ...
            if not os.path.isfile(filename):
                ...
                dataloader = torch.utils.data.DataLoader(
                    # self.FilterDataset(self.paths[:300]),
    ...
    class PrepareDataset:
        ...
        def __getitem__(self, idx):
            ...
            # Extract Landmarks
            if self.version == "LRS2":
                landmarks_pathname = file_path.replace(".txt", ".pkl").replace("LRS2/videos", "LRS2_landmarks")
            elif self.version == "LRS3":
    ...
    def prepare(self):
        ...
        if self.version == "LRS2":
            ...
            # paths_txt = glob.glob(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos", "*.txt"))
            
            paths_txt = glob.glob(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos/pretrain", "*", "*.txt")) + glob.glob(os.path.join("/data/conggaoxiang/Lip_Reading/LRS2/videos/main", "*", "*.txt"))
        elif self.version == "LRS3":
            paths_txt = glob.glob(os.path.join(self.root, "LRS3", "*", "*", "*.txt"))

        # Create Corpus File
        corpus_path = os.path.join(self.root, self.version, "corpus.txt") # 'datasets/LRS2/corpus.txt'
        # if not os.path.isfile(corpus_path):
        ...
        # Load Tokenizer 'datasets/LRS3/tokenizerbpe256.model'
...
class CorpusLM(Dataset):
    ...
    def __init__(self, batch_size, collate_fn, root="datasets", shuffle=True, download=False, tokenizer_path="datasets/LRS3/tokenizerbpe1024.model", max_length=None, corpus_path="datasets/LibriSpeechCorpus/librispeech-lm-norm.txt"):
        ...
        corpus_path = 'datasets/LRS3/corpus_lrs2_pretrain+train+val.txt'
