...
- `Set2_list`: Path to txt script of setting2, requried in running `Inference_Chem_Unbatch_New_S2.py` or `Inference_GRID_Unbatch_New_S2.py`. The intention is not to use the target audio as reference audio, the reference audio should come from another clip. `Set2_list` can be download [here](https://drive.google.com/drive/folders/1xYAwNrpDpER73v28o86NpC2GzgAhduiy?usp=sharing). 
