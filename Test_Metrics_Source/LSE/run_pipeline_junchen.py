...
import sys, time, pdb, argparse, pickle, subprocess, glob, cv2
import numpy as np
...
parser.add_argument('--data_dir',       type=str, default='tmp_dir', help='Output direcotry')
parser.add_argument('--videofile',      type=str, default='/data/conggaoxiang/Lip_Reading/0_Metrics/syncnet_python/GRID_GT_Test_MP4_1/s1-bbas3a.mp4',   help='Input video file')
parser.add_argument('--reference',      type=str, default='wav2lip',   help='Video reference')
...
print("opt.reference:", opt.reference)
...
input_audio_path = os.path.join("XXXX", '{}.wav'.format(opt.videofile.split('/')[-1].split('.mp4')[0])) 
# 

output_audio_path = os.path.join(opt.avi_dir, opt.reference, 'audio.wav')
command = ("ffmpeg -y -i %s -ac 1 -ar 16000 %s" % (input_audio_path, output_audio_path))
output = subprocess.call(command, shell=True, stdout=None)
